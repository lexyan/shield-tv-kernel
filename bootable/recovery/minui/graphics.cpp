...
void gr_blend(GRSurface* source, int sx, int sy, int w, int h, int dx, int dy) {
    ...
    if (gr_draw->pixel_bytes != 4 || source->pixel_bytes != 4) {
        printf("gr_blend: only RGBA8888 supported\n");
        return;
    }
    ...
    for (int y = 0; y < h; y++) {
        uint32_t *ip = (uint32_t *)src_p;
        uint32_t *op = (uint32_t *)dst_p;

        for (int x = 0; x < w; x++, ip++, op++) {
